...
export const Grid: React.FC = () => {
  ...
  const handleKeyDown = useCallback((e: KeyboardEvent) => {
    ...
    switch (e.key) {
      case 'ArrowUp':
        ...
        break;
      case 'ArrowDown':
        if (row < data.length - 1) setSelectedCell(row + 1, col);
        else if (row === data.length - 1) addRow();
        break;
      case 'ArrowLeft':
  ...
  }, [selectedCell, data.length, columns.length, setSelectedCell, addRow]);
      ...
      <div className="inline-block min-w-full">
        <div className="grid" style={{
          ...
          {columns.map((column, index) => (
            <div
              key={column.id}
              className="sticky top-0 z-10 bg-white dark:bg-gray-800 border-b border-r border-gray-200 dark:border-gray-700 group"
          ...
          {data.map((row, rowIndex) => (
            <React.Fragment key={rowIndex}>
              <div className="sticky left-0 bg-white dark:bg-gray-800 border-b border-r border-gray-200 dark:border-gray-700 h-9 flex items-center justify-center text-sm text-gray-500 dark:text-gray-400">
                {rowIndex + 1}
              </div>
